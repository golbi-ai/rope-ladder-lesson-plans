<!-- rope-ladder:begin document b8a7397589c578fa08473fc4ecce7b29524b529f624bd19abeebb20e0eeb792b -->
# Survival guide
...
These entries cover vendored third-party libraries. They are grouped here so the mainline guide stays focused on this repository's own code.

### accesskit: Keep vendored subsystem boundaries distinct
...
### basis_universal: Separate codec direction
...
### basis_universal: Start at container boundaries
...
### basis_universal: Treat state as caller-owned workspace
...
### clipper2: Follow Clipper Execute outputs
...
### embree: Follow filters at accepted hits
...
### embree: Read configuration before assuming a path is active
...
### embree: Read subdivision from basis to grid
...
### embree: Separate algorithms from packet width
...
### embree: Separate build from query
...
### embree: Start at the public handles
...
### embree: Trace primitive data first
...
### embree: Treat RTCRayHit as the query contract
...
### embree: Use geometry type to find leaf behavior
...
### enet: Follow ENet state through protocol.c
...
### enet: Treat enet_godot.cpp as the integration boundary
...
### etcpak: Use public codec headers as the texture-codec map
...
### freetype: Follow stream ownership carefully
...
### freetype: Recognize FreeType aggregation units
...
### freetype: Separate SFNT loading from validation
...
### freetype: Start at component assemblers
...
### freetype: Treat the cache as borrowed storage
...
### gamepadmotionhelpers: Read motion as a stateful update pipeline
...
### glslang: Follow shader translation from AST to blocks
...
### glslang: Start from public shader objects
...
### graphite: Follow a Graphite result through Segment and Slot
...
### graphite: Recognize the Graphite bridge as an adapter
...
### graphite: Treat Graphite behavior as font data
...
### harfbuzz: Model HarfBuzz shaping as buffer mutation
...
### harfbuzz: Separate HarfBuzz paint from its output backends
...
### harfbuzz: Start at table routing for HarfBuzz subsetting
...
### harfbuzz: Treat subset input and plan as the subsetting spine
...
### icu4c: Follow converter dispatch through shared data
...
### icu4c: Read ICU from data structures outward
...
### icu4c: Use LSR to understand locale matching
...
### jolt_physics: Bodies are manager-owned
...
### jolt_physics: Concurrency is opt-in through jobs
...
### jolt_physics: Contacts are solver data
...
### jolt_physics: Extensions reuse core simulation concepts
...
### jolt_physics: Keep Jolt AABB construction isolated
...
### jolt_physics: Platform performance paths are conditional
...
### jolt_physics: Serialization is explicit and type-aware
...
### jolt_physics: Shapes drive collision capability
...
### jolt_physics: Start at the simulation façade
...
### libjpeg-turbo: JPEG processing is a module graph
...
### libjpeg-turbo: Optimization paths are configuration-dependent
...
### libjpeg-turbo: Treat this as vendored media code
...
### libktx: KTX2 levels are offset-indexed
...
### libogg: Ogg is the codec boundary
...
### libwebp: Animation is canvas-state optimization
...
### libwebp: Choose the codec branch early
...
### libwebp: Read VP8L as transforms then symbols
...
### libwebp: Respect strides in every pixel path
...
### libwebp: Separate image coding from container work
...
### libwebp: Start at the public boundary
...
### libwebp: Treat WebPPicture as the ownership boundary
...
### libwebp: Use scalar DSP as the algorithmic baseline
...
### libwebp: WebP has three execution layers
...
### manifold: Start at the geometry boundary
...
### manifold: Treat CSG as a tree plus a mesh kernel
...
### mbedtls: Separate PSA policy from TLS protocol flow
...
### meshoptimizer: Read meshoptimizer by pipeline stage
...
### metal-cpp: Trace the Metal bridge before platform graphics behavior
...
### miniupnpc: Treat UPnP results as linked parser-owned data
...
### msdfgen: Separate MSDF generation from its repair pass
...
### openxr: Read OpenXR manifests before dispatch forwarding
...
### pcre2: Follow PCRE2 JIT into SLJIT only after compilation
...
### recastnavigation: Keep the Recast build stages in order
...
### recastnavigation: Start with the independent vendor boundaries
...
Related topics: [`navigation-regions`](../references/domain-concepts.md#navigation-region-construction), [`spirv-intermediate-representation`](../references/domain-concepts.md#spir-v-intermediate-representation)

### rvo2: Follow avoidance from neighbors to velocity
...
### sdl: Treat SDL events as owned queue records
...
### sdl: Use properties as shared SDL metadata
...
### spirv-cross: Choose the reflection implementation deliberately
...
Related topics: [`shader-reflection`](../references/domain-concepts.md#shader-reflection), [`spirv-intermediate-representation`](../references/domain-concepts.md#spir-v-intermediate-representation)

### spirv-cross: Keep shader IR separate from output back ends
...
Related topics: [`spirv-intermediate-representation`](../references/domain-concepts.md#spir-v-intermediate-representation)

### spirv-reflect: Read the SPIR-V module first
...
### thorvg: Check feature macros before tracing ThorVG paths
...
### thorvg: Treat render work as tasks
...
### ufbx: Separate imported assets from geometry processing
...
### vulkan: Find Godot deltas before reading generated code
...
### vulkan: Follow pNext deliberately
...
### vulkan: Read shared ownership from the control block outward
...
### vulkan: Read the C declaration layer first
...
### vulkan: Read the records as API contracts
...
### vulkan: Separate the C ABI from C++ conveniences
...
### vulkan: Start Vulkan window integration at creation descriptors
...
### vulkan: Start with the C versus C++ split
...
### vulkan: Treat pNext as a structural convention
...
### vulkan: Treat platform create-info structs as external API contracts
...
### vulkan: Use the C++ façade as a separate API layer
...
### wayland-protocols: Treat Wayland XML as protocol state machines
...
### wslay: Separate WebSocket framing from event dispatch
...
### xatlas: Follow xatlas from MeshDecl to charts and packing
...
### zlib: Read compression by public state then algorithm modules
